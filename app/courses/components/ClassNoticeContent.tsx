...
export default function ClassNoticeContent({ onBack }: ClassNoticeContentProps) {
  ...
  const fetchNotices = async () => {
    try {
      setLoading(true);
      const response = await classNoticeApi.getNotices();
      ...
      <ClassNoticeWrite
        ...
        onSave={async (formData) => {
          try {
            ...
            await classNoticeApi.createNotice(formData, token);
            setIsWriting(false);
            await fetchNotices();
          } catch (error) {
              ...
              {paginatedItems.length > 0 ? (
                paginatedItems.map((item, index) => (
                  <tr
                    ...
                    onClick={async () => {
                      try {
                        const response = await classNoticeApi.getNotice(item.id);
                        setSelectedNotice(response.data.notice || response.data);
                      } catch (error) {
